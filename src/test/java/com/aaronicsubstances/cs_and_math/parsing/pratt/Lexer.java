 ...
 */
public class Lexer implements Iterator<DefaultToken> {
    private final Map<Character, TokenType> mPunctuators = new HashMap<>();
    ...
    @Override
    public DefaultToken next() {
        while (mIndex < mText.length()) {
            char c = mText.charAt(mIndex++);
            
            if (mPunctuators.containsKey(c)) {
                // Handle punctuation.
                return new DefaultToken(mPunctuators.get(c), Character.toString(c));
                ...
                return new DefaultToken(TokenType.NAME, name);
            } else {
        ...
        return new DefaultToken(TokenType.EOF, "");
    }
